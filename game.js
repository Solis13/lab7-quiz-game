...
let url = 'https://api.worldbank.org/v2/country/pe?format=json'

fetch(url).then( res=> {  // res=response(is a function)
    return res.json()// response to json
...
.then((countriesAndCodes) =>{
console.log(countriesAndCodes) 
// You don't need to log countriesAndCodes - just proving it is available 

let randomCountry = countriesAndCodes.randomCountryElement
let user = countriesAndCodes.userAnswerElement
let submitAnwer = countriesAndCodes.submitButton
let result = countriesAndCodes.resultTextElement

countriesAndCodes.innerHTML=randomCountry
countriesAndCodes.innerHTML=user
countriesAndCodes.innerHTML=submitAnwer
countriesAndCodes.innerHTML=result
...
// TODO add a click event handler to the submitButton.  When the user clicks the button,
 ...
 submitButton.addEventListener('Click', function(){
    let userAnswerElement = userAnswerElement.value
    let capitalCity = capitalCity.value
    
    if(userAnswerElement === userAnswerElement.value){
        resultTextElement.textContent = `Correct the capital of ${randomCountry.name} if ${capitalCity}`
    } else 
    {
        resultTextElement.textContent = `Wrong the capital of ${randomCountry.name} if ${capitalCity}`
    }
